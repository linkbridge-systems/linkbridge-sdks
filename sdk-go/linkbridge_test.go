...
func TestSubmit_HappyPath(t *testing.T) {
	...
	out, err := c.Invoices.Submit(context.Background(),
		oapi.Invoice{"irn": "INV-1"}, SubmitOptions{IdempotencyKey: "fixed-key"})
	if err != nil {
	...
	if _, err := c.Invoices.Submit(context.Background(), oapi.Invoice{"irn": "INV-2"}, SubmitOptions{}); err != nil {
...
func TestSubmit_AutoIdempotencyKey(t *testing.T) {
	...
	if _, err := c.Invoices.Submit(context.Background(), oapi.Invoice{"irn": "X"}, SubmitOptions{}); err != nil {
		t.Fatalf("Submit: %v", err)
	}
...
func TestSubmit_SyncPreservesResultFields(t *testing.T) {
	...
	out, err := c.Invoices.Submit(context.Background(), oapi.Invoice{"irn": "INV-1"}, SubmitOptions{Mode: "sync"})
	if err != nil {
		t.Fatalf("Submit: %v", err)
	}
...
func TestSubmit_APIError(t *testing.T) {
	...
	_, err := c.Invoices.Submit(context.Background(), oapi.Invoice{}, SubmitOptions{IdempotencyKey: "k"})
	var apiErr *APIError
...
func TestStaticTokenBypassesTokenEndpoint(t *testing.T) {
	...
	if _, err := c.Invoices.Submit(context.Background(), oapi.Invoice{}, SubmitOptions{IdempotencyKey: "k"}); err != nil {
		t.Fatalf("Submit: %v", err)
	}
	if f.tokenCalls != 0 {
...
func TestTokenRefreshOnExpiry(t *testing.T) {
	...
	_, _ = c.Invoices.Submit(context.Background(), oapi.Invoice{}, SubmitOptions{IdempotencyKey: "k"})
	...
	_, _ = c.Invoices.Submit(context.Background(), oapi.Invoice{}, SubmitOptions{IdempotencyKey: "k2"})
	if calls < 2 {
